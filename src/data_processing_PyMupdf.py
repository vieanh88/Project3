...
import fitz  # PyMuPDF
import csv # Để xử lý lưu CSV với escapechar
...
class PDFDataProcessor:
    ...
    def extract_text_from_pdf(self, pdf_path, max_chars=5000):
        ...
        try:
            ...
            # Clean text cơ bản
            text = text.strip()
            # Xử lý lỗi phổ biến: nhiều PDF tiếng Việt bị tách dấu sai (vd: e ^ n -> ên)
            # PyMuPDF thường xử lý cái này tốt hơn, nhưng ta cứ normalize lại
            import unicodedata
    ...
    def create_train_val_test_split(self, df, test_size=0.15, val_size=0.15, random_state=42):
        ...
        train_df.to_csv(splits_dir / 'train.csv', index=False, encoding='utf-8', quoting=csv.QUOTE_ALL, escapechar='\\')
        val_df.to_csv(splits_dir / 'val.csv', index=False, encoding='utf-8', quoting=csv.QUOTE_ALL, escapechar='\\')
        test_df.to_csv(splits_dir / 'test.csv', index=False, encoding='utf-8', quoting=csv.QUOTE_ALL, escapechar='\\')
...
def main():
    ...
    df.to_csv(full_dataset_path, index=False, encoding='utf-8', quoting=csv.QUOTE_ALL, escapechar='\\')
